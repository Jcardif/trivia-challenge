...
export default function OperatorGate({ children }: { children: ReactNode }) {
  ...
  const handleSignOut = () => {
    if (!connection || busy || unfinishedGame || isPlaying) return
    setBusy(true)
    setError(null)
  ...
  return (
    <>
      ...
      {showSetup && (
        <div className="fixed inset-0 z-[100] overflow-y-auto bg-[#040406] text-white" role={isOperatorPage ? 'main' : 'dialog'} aria-modal={isOperatorPage ? undefined : true} aria-labelledby="operator-heading" onKeyDownCapture={event => event.stopPropagation()}>
          ...
          <div className="relative mx-auto flex min-h-screen max-w-xl items-center px-5 py-12">
            <section className="w-full rounded-[34px] border border-amber-200/20 bg-white/5 p-9 text-center shadow-[0_32px_64px_rgba(0,0,0,0.65)]">
              ...
              <h1 id="operator-heading" className="text-2xl font-semibold">{isOperatorPage ? 'Kiosk operator setup' : 'Operator sign-in required'}</h1>
              <p className="mt-4 text-white/70">
                {isOperatorPage
                  ? 'Sign in once with Fabric for this browser. Attendees use the registration form and do not need Fabric accounts.'
                  : 'Please contact an operator to sign in with Fabric. Attendees do not need Fabric accounts.'}
              </p>
              ...
              <div className="mt-7 flex flex-col gap-3">
                ...
                {isOperatorPage && authenticated && !authFailure && (
                  <>
                    <p className="text-sm text-emerald-300">Operator signed in. This session is retained between attendees.</p>
                    <Link to={session ? (isPlaying ? '/playing' : '/results') : '/signin'} className="rounded-2xl bg-amber-400 px-6 py-3 font-semibold text-black">
                      Continue to the challenge
                    </Link>
                    {!unfinishedGame && !isPlaying && (
                      <Link to="/questions/load" className="rounded-2xl border border-white/20 px-6 py-3 font-semibold hover:bg-white/10">
                        Load questions and create pools
                      </Link>
                    )}
                  </>
                )}
                {isOperatorPage && authenticated && (
                  <button type="button" onClick={handleSignOut} disabled={busy || unfinishedGame || isPlaying} className="rounded-2xl border border-white/20 px-6 py-3 text-sm disabled:opacity-50">
                    Sign out operator
                  </button>
                )}
                {isOperatorPage && unfinishedGame && <p className="text-xs text-white/60">Finish and save the current game before loading questions or signing out. Save status: {saveState.status}.</p>}
              </div>
              {isOperatorPage && authenticated && !authFailure && (
