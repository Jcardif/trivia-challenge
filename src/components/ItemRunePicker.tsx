...
const icons: Record<RuneId, ComponentType<SVGProps<SVGSVGElement>>> = {
  ...
  report: Report32Item,
  'semantic-model': SemanticModel32Item,
  'ml-model': Model32Item,
}
...
export default function ItemRunePicker({
  value,
  onChange,
  disabled = false,
  availableRunes,
}: RunePickerProps & { availableRunes?: readonly RuneId[] }) {
  const runes = availableRunes
    ? ITEM_RUNES.filter(rune => availableRunes.includes(rune.id))
    : ITEM_RUNES
  return (
