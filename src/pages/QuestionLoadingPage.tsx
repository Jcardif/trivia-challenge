...
import type { ImportQuestionsInput, ImportQuestionsResponse, QuestionPool } from '../types/api'
...
export default function QuestionLoadingPage() {
  ...
  const selectFile = async (file: File | undefined) => {
    ...
    if (!file) { setPhase('empty'); return }
    if (file.size > MAX_CSV_BYTES) {
      setImportError('CSV files must be no larger than 10 MiB.')
      setPhase('error')
      return
    }
    setPhase('reading')
    try {
      ...
      setPendingImport({ importId: crypto.randomUUID(), csv })
      setPhase('ready')
    } catch (error: unknown) {
      if (!mountedRef.current || version !== fileVersionRef.current) return
      setImportError(error instanceof Error ? error.message : 'The CSV file could not be read.')
      setPhase('error')
    }
  }

  const importQuestions = async () => {
    if (!pendingImport || importBusyRef.current || lockdown || phase === 'complete') return
    ...
    setValidationErrors([])
    try {
      const imported = await sessionService.importQuestions(pendingImport)
      if (!mountedRef.current || version !== fileVersionRef.current) return
      ...
      <main className="relative mx-auto max-w-5xl px-5 py-10">
        ...
        <div className="mt-7 grid items-start gap-7 md:grid-cols-2">
          <section className="rounded-3xl border border-amber-200/20 bg-white/5 p-7">
            ...
            {!loadingPools && pools.length === 0 && <p className="mt-4 text-white/60">No active pools yet. Create one below.</p>}
          ...
          <section className="rounded-3xl border border-amber-200/20 bg-white/5 p-7">
            ...
            <p className="mt-3 text-sm text-white/65">The Pools column assigns comma-separated slugs. Quote cells containing commas. A blank Pools value uses default. Pool display records can be created before or after importing.</p>
            <label className="mt-6 block text-sm">Question file<input type="file" accept=".csv,text/csv" disabled={importBusy || lockdown} onChange={event => { void selectFile(event.target.files?.[0]) }} className={`${inputClass} file:mr-4 file:rounded-lg file:border-0 file:bg-white/15 file:px-3 file:py-1 file:text-white`} /></label>
            ...
            {importBusy && <div className="mt-5" role="status"><p>{phase === 'reading' ? 'Reading the CSV file...' : 'Validating and saving questions in Fabric...'}</p><progress aria-label="CSV import progress" className="mt-3 w-full" /><p className="mt-2 text-sm text-white/60">Keep this tab open until the operation finishes.</p></div>}
            ...
            <button type="button" disabled={!pendingImport || importBusy || phase === 'complete' || lockdown} onClick={() => { void importQuestions() }} className={`${buttonClass} mt-6`}>
              {phase === 'error' && pendingImport ? 'Retry this import' : 'Import questions'}
            </button>
            <p className="mt-3 text-xs text-white/50">Retry keeps the same import ID and cannot add the same import twice. Selecting a file starts a new import.</p>
          </section>
        </div>
      </main>
    </div>
