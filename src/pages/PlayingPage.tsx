...
export default function PlayingPage() {
  ...
  const latestTimeRef = useRef<number>(gameConfig.timer.initialSeconds)
  ...
  const initializeGame = useCallback(async () => {
    ...
    const isCurrent = () => isMountedRef.current && isCurrentGame(generation) &&
      initializationRef.current === initialization

    try {
      const newSession = await sessionService.start(startRequestId, player.userId, selectedPool?.id)
      if (!isCurrent()) return
  ...
  }, [player, selectedPool?.id, startRequestId, generation, isCurrentGame, beginSession, setQuestions, setCurrentQuestionIndex, setQuestionsAnswered, setCorrectAnswers, setScore, setCurrentStreak, setStreaksCompleted, setHearts, setGameOverReason, setIsPlaying, setMissedQuestions, startCountdown, lockdownMessage])
  ...
  const handleSessionEnd = useCallback((
    ...
    if (sessionEndedRef.current || !session || !isCurrentGame(generation)) return
    ...
    completeGame(summary)
    navigate('/results', { replace: true })
  }, [session, gameOverReason, completeGame, navigate, generation, isCurrentGame, clearTimeoutRef])

  useEffect(() => {
    onTimeUpRef.current = () => {
      handleSessionEnd({ finalTimeRemaining: 0 })
    }
  }, [handleSessionEnd])
  ...
  const goToNextQuestion = useCallback(() => {
    ...
    if (nextIndex >= questions.length) {
      handleSessionEnd({
        ...
        finalTimeRemaining: latestTimeRef.current,
      })
      return
    }
    setCurrentQuestionIndex(nextIndex)
    // The questionDisplayTimeRef will be updated by the effect when the new question renders
  }, [currentQuestionIndex, questions.length, handleSessionEnd, setCurrentQuestionIndex])

  const handleQuestionProgress = useCallback(() => {
    if (sessionEndedRef.current || !isCurrentGame(generation)) return
    clearTimeoutRef(wrongAnswerTimeoutRef)
    clearTimeoutRef(correctAnswerTimeoutRef)
  ...
  }, [clearTimeoutRef, goToNextQuestion, isCurrentGame, generation])
  ...
  const handleAnswerSelect = useCallback((answerIndex: number) => {
    if (!session || !currentQuestion || answerLockedRef.current || sessionEndedRef.current ||
        !isCurrentGame(generation) || isSubmitting || timerState !== 'running') {
      return
    }
    answerLockedRef.current = true
    ...
    queueAnswer({
        ...
        if (!isCurrentGame(generation) || answerTracked) return
        answerTracked = true
        analytics.track(
          'game.answerquestion',
          {
        ...
        if (!isCurrentGame(generation) || answerTracked) return
        answerTracked = true
        analytics.track('game.answerquestion', {
          ...baseTelemetry,
          apiSuccess: false,
          error: message,
        }, { page: 'playing' })
    ...
    if (isCorrect) {
      ...
      const advanceAfterHalo = () => {
        if (!isMountedRef.current || !isCurrentGame(generation) || sessionEndedRef.current) {
          return
        }
      ...
      <div className="relative min-h-screen overflow-hidden bg-[#040406] text-white">
        ...
        <div className="relative z-10 flex min-h-screen flex-col">
          ...
          <div className="flex flex-1 flex-col items-center justify-center space-y-6 px-6 text-center">
            ...
            <p className="text-sm text-white/60">The same game ID is reused when you retry.</p>
            {!session && (
              <button type="button" onClick={() => { setIsPlaying(false); navigate('/select-pool', { replace: true }) }} className="text-sm text-white/70 underline">
                Choose another question pool
              </button>
            )}
          </div>
        </div>
    ...
    <div className="relative min-h-screen overflow-hidden bg-[#040406] text-white">
      ...
      <div className="relative z-10 flex min-h-screen flex-col">
        ...
        {saveState.status === 'error' && (
          <div className="mx-auto mb-3 max-w-3xl rounded-xl border border-amber-400/50 bg-amber-950/80 px-4 py-3 text-sm" role="alert">
            Answers are waiting to be saved. {saveState.error}{' '}
            <button type="button" onClick={retrySave} className="font-semibold underline">
              Retry saving
            </button>
          </div>
        )}
