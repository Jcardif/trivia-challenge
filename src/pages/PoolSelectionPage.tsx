...
export default function PoolSelectionPage() {
  ...
  const { player, setSelectedPool, generation, isCurrentGame } = useGame()
  ...
  useEffect(() => {
    ...
    const loadPools = async () => {
      try {
        ...
        if (!current || !isCurrentGame(generation)) return
        ...
        if (current && isCurrentGame(generation)) setIsLoading(false)
      }
    }

    void loadPools()
    return () => { current = false }
  }, [player, navigate, setSelectedPool, generation, isCurrentGame, attempt])
            ...
            <div className="relative overflow-hidden rounded-4xl bg-neutral-950/80 px-10 pb-12 pt-14">
              ...
              {displayedError && (
                <div className="relative mt-8 rounded-xl border border-red-500/50 bg-red-500/10 px-4 py-3 text-center text-sm text-red-200">
                  {displayedError}
                  {!isLockdownActive && <button type="button" onClick={() => setAttempt(value => value + 1)} className="ml-3 font-semibold underline">Retry loading pools</button>}
                </div>
              )}
